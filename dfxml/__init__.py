...
rx_rfc822datetime = re.compile(
    r"(?P<day>\d{1,2}) (?P<month>Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (?P<year>\d{4}) (?P<hours>\d\d):(?P<minutes>\d\d):(?P<seconds>\d\d) (?P<timezone>Z|[-+]\d\d:?\d\d)"
)
...
class byte_run:
    ...
    def decode_sax_attributes(self, attr):
        for key, value in attr.items():
            if key == "bytes":
                key == "len"  # tag changed name; provide backwards compatability
            try:
                setattr(self, key, int(value))
            except ValueError:
                setattr(self, key, value)
...
class dftime(ComparableMixin):
    ...
    def __init__(self, val):
        ...
        if isinstance(val, str) or isinstance(val, _basestring):
            ...
            raise ValueError(
                "Unknown type '%s' for DFXML time value" % (str(type(val)))
            )
    ...
    def iso8601(self):
        ...
        try:
            ...
            self.iso8601_ = time.strftime(
                "%Y-%m-%dT%H:%M:%SZ", time.gmtime(self.timestamp_)
            )
            return self.iso8601_
...
class fileobject:
    ...
    def __str__(self):
        try:
            ...
            fn = "???"
        return "fileobject %s byte_runs: %s" % (
            fn,
            " ".join([str(x) for x in self.byte_runs()]),
        )
    ...
    def allocated(self):
        ...
        """
        if self.filename() == "$OrphanFiles":
            return False
        if self.allocated_inode() and self.allocated_name():
            return True
        else:
            return (
                isone(self.tag("alloc"))
                or isone(self.tag("ALLOC"))
                or not isone(self.tag("unalloc"))
            )

    def compressed(self):
        if not self.has_tag("compressed") and not self.has_tag("compressed"):
            return False
        return isone(self.tag("compressed")) or isone(self.tag("COMPRESSED"))

    def encrypted(self):
        if not self.has_tag("encrypted") and not self.has_tag("encrypted"):
            return False
    ...
    def file_present(self, imagefile=None):
        ...
        for hashname in ["md5", "sha1", "sha256"]:
            ...
            if oldhash:
                newhash = hashlib.new(
                    hashname, self.contents(imagefile=imagefile)
                ).hexdigest()
                return oldhash == newhash
        raise ValueError(
            "Cannot process file " + self.filename() + ": no hash in " + str(self)
        )
    ...
    def content_for_run(self, run=None, imagefile=None):
        ...
        if run is None:
            raise ValueError("content_for_run called without a 'run' argument.")
    ...
    def contents(self, imagefile=None, icat_fallback=True):
        ...
        if self.encrypted():
            raise ValueError("Cannot generate content for encrypted files")
            ...
            if icat_fallback:
                ...
                if inode:
                    ...
                    if len(err) > 0:
                        ...
                        raise ValueError(
                            "icat error (" + str(err).strip() + "): " + " ".join(cmd)
                        )
                    return data
                else:
    ...
    def tempfile(self, calcMD5=False, calcSHA1=False, calcSHA256=False):
        ...
        if calcMD5:
            tf.md5 = hashlib.md5()
        if calcSHA1:
            tf.sha1 = hashlib.sha1()
        if calcSHA256:
            tf.sha256 = hashlib.sha256()
        for run in self.byte_runs():
            ...
            while count > 0:
                xfer_len = min(
                    count, 1024 * 1024
                )  # transfer up to a megabyte at a time
...
def safe_b64decode(b64data):
    ...
    if sys.version_info.major == 2:
        ...
        raise Exception(
            "Not sure how to parse base64 data outside Python versions 2 or 3."
        )
        ...
        * value:  Create a new value object, append to objectstack.
        ...
        if name in ["msregistry", "hive"]:
            ...
            if new_object.parent_key == None:
                ...
                new_object._full_path = (
                    new_object.parent_key.full_path() + "\\" + new_object.name()
                )
            self.objectstack.append(new_object)
        elif name in ["value"]:
            ...
            if attrs.get("default", None) == "1":
                ...
                if enc == "base64":
                    try:
                        new_object._name = base64.b64decode(
                            name_data.encode("ascii")
            ...
            parent._byte_runs.append(
                byte_run(file_offset=attrs.get("file_offset"), len=attrs.get("len"))
            )
        else:
            raise ValueError(
                "regxml_reader._start_element: Don't know how to start element %s.\n"
                % name
            )
    ...
    def _end_element(self, name):
        ...
        if name in ["msregistry", "hive"]:
            ...
            if finished_object.full_path() in self.registry_object.object_index:
                raise ValueError(
                    "regxml_reader._end_element:  Same key path found more than once: "
                    + finished_object.full_path()
                )
            self.registry_object.object_index[finished_object.full_path()] = (
                finished_object
            )
            ...
            if value_object.strings == None:
                raise ValueError(
                    "regxml_reader._end_element:  parsing error, string element found, but parent's type can't support a string list."
                )
            ...
            raise ValueError(
                "regxml_reader._end_element: Don't know how to end element %s.\n" % name
            )
...
class fileobject_reader(xml_reader):
    ...
    def _start_element(self, name, attrs):
        ...
        if name == "original_fileobject":
            self.fileobject.original_fileobject = fileobject_sax(
                imagefile=self.imagefile
            )
...
def combine_runs(runs):
    ...
    for run in runs[1:]:
        ...
        if last.img_offset + last.len == run.img_offset:
            ret[-1] = byte_run(img_offset=last.img_offset, len=last.len + run.len)
            continue
        else:
            ret.append(run)
    return ret
...
class extentdb:
    ...
    def run_for_sector(self, sector_number, count=1):
        ...
        return byte_run(
            len=count * self.sectorsize, img_offset=sector_number * self.sectorsize
        )

    def intersects(self, extent):
        """Returns the intersecting extent, or None if there is none"""
        if extent.len == 0:
            return True  # 0 length intersects with everything
        if extent.len < 0:
            raise ValueError("Length cannot be negative:" + str(extent))
        start = extent.img_offset
        stop = extent.img_offset + extent.len
        for d in self.db:
            if d.img_offset <= start < d.img_offset + d.len:
                return d
            if d.img_offset < stop < d.img_offset + d.len:
                return d
            if start < d.img_offset and d.img_offset + d.len <= stop:
                return d
        return None
    ...
    def runs_for_sectors(self, sectors):
        ...
        runs = [
            byte_run(len=self.sectorsize, img_offset=x * self.sectorsize)
            for x in sectors
        ]
    ...
    def sectors_not_in_db(self, run):
        ...
        return filter(
            lambda x: not self.intersects_sector(x), self.sectors_for_run(run)
        )


def read_dfxml(
    xmlfile=None, imagefile=None, flags=0, callback=None, preserve_fis=False
) -> fileobject_reader:
...
def read_regxml(xmlfile=None, flags=0, callback=None):
    ...
    try:
        ...
        stderr.write(
            'XML parsing error for file "' + xmlfile.name + '".  Object stack:\n'
        )
