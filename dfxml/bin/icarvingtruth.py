...
def add_reference(doc, image="", reason=""):
    d2 = parseString(
        "<reference><image>%s</image><reason>%s</reason></reference>" % (image, reason)
    )
...
def print_ground_truth_fi(fi, out=sys.stdout):
    ...
    for run in fi.byte_runs():
        ...
        out.write(
            "%10s @ sector %8d ; %5d %4d-byte sectors %14s (%7d bytes total)\n"
            % (
                desc,
                run.start_sector(),
                run.sector_count(),
...
def print_ground_truth_report(doc, out=sys.stdout):
    def sort_by_runs(a, b):
        if len(a.byte_runs()) == 0:
            return -1
        if len(b.byte_runs()) == 0:
            return 1
        if a.byte_runs()[0].img_offset < b.byte_runs()[0].img_offset:
            return -1
        if a.byte_runs()[0].img_offset > b.byte_runs()[0].img_offset:
            return 1
        return 0

    filist = [dfxml.fileobject_dom(x) for x in doc.getElementsByTagName("fileobject")]
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-r",
        "--report",
        help="specify output file for the report",
        dest="reportfilename",
    )
    ...
    for node in doc.childNodes[0].childNodes:
        if node.nodeType == node.ELEMENT_NODE and node.nodeName not in [
            "volume",
            "fileobject",
        ]:
    ...
    for ref in refs:
        ...
        for fi in fobj2:
            for run in fi.byte_runs():
                ...
                def check_sector(n):
                    return sector_from_file(ref_imagefile, n) == sector_from_file(
                        master_imagefile, n
                    )
                ...
                if sectors_that_match:
                    if options.debug:
                        print(
                            ...
                            "run sectors:",
                            db.sectors_for_bytes(run.len),
                            "total sectors: ",
                            len(sectors_to_check),
                            "matching:",
                            len(sectors_that_match),
                        )
