...
def _lower_ftype_str(vo: Objects.VolumeObject) -> typing.Optional[str]:
    """
...
def ignorable_name(fn: str) -> bool:
...
def make_differential_dfxml(
    ...
    *,
    ...
    Signature_fis_unalloc = typing.Dict[
        typing.Tuple[typing.Optional[int], typing.Optional[int], typing.Optional[str]],
        typing.List[Objects.FileObject],
    ]
    ...
    # Value: VolumeObject
    Signature_volumes = typing.Dict[
        typing.Tuple[int, typing.Optional[str]], Objects.VolumeObject
    ]
    ...
    volumes_encounter_order: typing.Dict[
        typing.Tuple[int, typing.Optional[str]], int
    ] = dict()
    ...
    for infile in [pre, post]:
        ...
        for i, (event, new_obj) in enumerate(Objects.iterparse(infile)):
            if isinstance(new_obj, Objects.DFXMLObject):
                ...
                # 2. If the volume is NOT in the old list, add it to the new list.
                    ...
                    volumes_encounter_order[(offset, ftype_str)] = (
                        len(new_volumes)
                        + ((old_volumes and len(old_volumes)) or 0)
                        + len(matched_volumes)
                    )
            ...
            _key_partition = (
                None if "partition" in ignore_properties else new_obj.partition
            )
            _key_inode = None if "inode" in ignore_properties else new_obj.inode
            _key_filename = (
                None if "filename" in ignore_properties else new_obj.filename
            )
        ...
        for new_inode_name_key in new_inode_names.keys():
            ...
            if rename_requires_hash:
                ...
                old_fobj = old_fis[
                    (partition, inode, list(old_inode_names[new_inode_name_key])[0])
                ]
                new_obj = new_fis[
                    (partition, inode, list(new_inode_names[new_inode_name_key])[0])
                ]
        ...
        def _maybe_match_attr(obj: Objects.FileObject) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--idifference-diffs",
        action="store_true",
        help="Only consider the modifications idifference had considered (names, hashes, timestamps).",
    )
    parser.add_argument(
        "-i",
        "--ignore",
        action="append",
        help="Object property to ignore in all difference operations.  E.g. pass '-i inode' to ignore inode differences when comparing directory trees on the same file system.  Affects annotation attributes placed on the fileobject element and the named property's elements, and also the properties used to determine file identities for matching.",
    )
    parser.add_argument(
        "--rename-with-hash",
        action="store_true",
        help="Require that renamed files must match on a content hash.",
    )
    parser.add_argument(
        "--retain-unchanged",
        action="store_true",
        help="Output unchanged files in the resulting DFXML file.",
        default=False,
    )
    parser.add_argument(
        "--annotate-matches",
        action="store_true",
        help="Add a 'delta:matched' Boolean attribute to every produced object.  Useful for some counting purposes, but not always needed.",
        default=False,
    )
    parser.add_argument(
        "--simplify-byte-runs",
        action="store_true",
        help="Join contiguous byte run elements together, if their attributes align.",
        default=False,
    )
