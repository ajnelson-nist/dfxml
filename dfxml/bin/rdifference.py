...
def dprint(x):
    global options
    if "options" in globals() and options.debug:
        print(x)
...
class HiveState:
    ...
    def process_cell(self, cell):
        ...
        if ocell:
            ...
            if ocell.mtime() != cell.mtime():
                ...
                self.changed_properties.add((ocell, cell))
            elif ((ocell.parent_key and ocell.parent_key.mtime()) or None) != (
                (cell.parent_key and cell.parent_key.mtime()) or None
            ):
    ...
    def process(self, fname):
        ...
        if fname.endswith(".regxml"):
            reader = dfxml.read_regxml(
                xmlfile=open(fname, "rb"), callback=self.process_cell
            )
    ...
    def print_cell2(self, title, cell2s):
        ...
        for ocell, cell in cell2s:
            if ocell.sha1() != cell.sha1():
                res.add(
                    (ocell.full_path(), "SHA1 changed", ocell.sha1(), "->", cell.sha1())
                )
                if self.timeline:
                    self.timeline.add(
                        (
