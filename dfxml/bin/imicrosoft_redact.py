...
def should_redact(fi):
    ...
    if ext in redact_extensions and fnl.startswith("windows"):
        try:
            ...
            if options.debug:
                print(" *** can't redact --- is compressed *** ")
            return False
        if not content:
            if options.debug:
                print(" *** can't redact --- no content ***")
            return False
        ...
        if "\0M\0i\0c\0r\0o\0s\0o\0f\0t" in content:
            return True
        if options.debug:
            print(" *** won't redact --- no Microsoft ***")
        return False
    return False
...
def redact(fi):
    ...
    if br.img_offset == 0:
        return  # this run isn't on the disk
    if br.bytes == 0:
        return  # too small to redact
    ...
    xml_out.write(
        "    <hashdigest type='SHA1'>%s</hashdigest>\n"
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-d", "--debug", help="prints debugging info", dest="debug", action="store_true"
    )
    parser.add_option(
